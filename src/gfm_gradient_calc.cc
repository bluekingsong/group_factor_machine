...
double GfmGradientCalc::operator()(const Real *w, Real * g){
    ...
    uint32_t dataSize = data->l, start = 0;
    if(sampleEnabled){
        dataSize = sampleSize;
        start = sampleIndex;
    }
    ...
    for(uint32_t i = start; i < start + dataSize && i < data->l; ++i){
        ...
        predictedProb[i] = ui;
        // preventing overflow
        if(yi)    t < -25 ? logloss -= weight * sampleWeight * t : logloss -= weight * sampleWeight * log(ui);
        else      t > 25  ? logloss += weight * sampleWeight * t : logloss -= weight * sampleWeight * log(1-ui);
    }
    groupLassoLoss = 0;
...
double GfmGradientCalc::calc_test_auc(const Problem& dataset){
    aucUti.clear();
    for(uint64_t i = 0; i < dataset.l; ++i){
        ...
        t += intercept;
        double ui = 1.0 / (1.0 + std::exp(-t));
        aucUti.add_instance(ui,dataset.y[i]);
    }
    double auc = aucUti.auc();
    aucUti.clear();
    return auc;
}
